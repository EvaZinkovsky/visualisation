...
figure. There are several ways you can perform this, but today we will be using the `patchwork` [package](https://github.com/thomasp85/patchwork)

> ## Installing `patchwork`
> `patchwork` is not available on CRAN and so `install.packages("patchwork")` will fail. Instead, you
> will need to install it directly from GitHub if you wish to try it out. To do this, you will first
> need to install the `remotes` package:
> ~~~~~~~
> install.packages("remotes")
> ~~~~~~~
> {:.language-r}
> which will let you install packages from a GitHub repository:
> ~~~~~~~
> remotes::install_github("thomasp85/patchwork")
> ~~~~~~~
> {:.language-r}
>
> Installing from GitHub can open up a wider range of functionality than is available through packages 
> on CRAN. However these packages have not gone through any vetting process, so there is no guarantee
> that they will function correctly or be well documented.
{:.callout}

Combining figures using `patchwork` is performed by adding them together:

```{r patchwork}
library(patchwork)

plot1 <- ggplot(gapminder, aes(x = gdpPercap, y = lifeExp)) + geom_point()
plot2 <- ggplot(gapminder, aes(x = lifeExp, y = pop)) + geom_point()
plot3 <- ggplot(gapminder, aes(x = continent, y = lifeExp)) + geom_boxplot()
plot4 <- ggplot(gapminder, aes(x = gdpPercap, y = pop)) + geom_point()

plot1 + plot2 + plot3 + plot4
```

You can control how the plots are laid out by adding a `plot_layout()` layer.

```{r patchwork_layout}
plot1 + plot2 + plot3 + plot4 + plot_layout(heights = c(2,1))
```

And label the subfigures by adding a `plot_annotation()` layer.

```{r patchwork_annot}
plot1 + plot2 + plot3 + plot4 + plot_annotation(title = "Collection of plots", tag_levels = "A")
```
...
>
> 1) Produce a scatter plot showing the relationship between each pair of the four meterological
> observations -- Minimum temperature, Maximum temperature, Rainfall, and Solar Exposure. You will 
> have six plots in total.
> 
> 2) Select four of these plots and save them as a multi-panel figure.
> 
> 3) Create a barplot showing the average monthly rainfall per state. Can you come up with a better 
> way to display this information?
>
> 4) Select a single station and year. Produce a plot showing the minimum and maximum temperature
> as well as the solar exposure over the year. Once you are satisfied with the plot for a single station,
> use all data from that year and facet the plot by station name.
{:.challenge}
